angular.module('starter.publishActivityControllers',[])
    .controller('activity',function($state,$scope,$rootScope,$stateParams,publishActivityServices,system,$ionicPopup){
      function initScope(){
        ...
        $rootScope.activitySaveInfo=function(){
            setTimeout(function(){
                system.bindNavTool({
                    rightText:"保存草稿",
                    toolClick:function () {
                        var promise=publishActivityServices.saveInfo($rootScope.activityEphemeralData,$stateParams.source);
                        if(promise) {
...
/*            if(!$rootScope.activityEphemeralData['报名截止时间']){
            ...
            if(!($rootScope.activityEphemeralData['线下活动地址']||($rootScope.activityEphemeralData['线上活动'] instanceof Boolean))){
                template+="活动方式未选择！<br>";
                showLayer=true;
            }
            ...
            //$state.current.name
            if($state.current.name=='chat-publish-activity'){
              //聊天界面活动发布
              alert('chat')
            }else {
                alert('publish')
              var promise = publishActivityServices.publishInfo($rootScope.activityEphemeralData,'publish');
            }
